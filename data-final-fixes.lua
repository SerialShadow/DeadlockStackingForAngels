...
local function walk_recipes()
    ...
    for _, recipe in pairs(data.raw.recipe) do
        if recipe.enabled then
            add_products_from_recipe(recipe, data.raw.technology[default_beltbox])
        end
    end
    log("hmm")
    for _, resource in pairs(data.raw.resource) do
        if resource.minable then
            add_products_from_recipe(resource.minable, data.raw.technology[default_beltbox])
        end
    end
    log("hmm")
    for name, _ in pairs(tech_by_product) do
        if #tech_by_product[name].tech == 0 then
            table.insert(tech_by_product[name].tech, data.raw.technology[default_beltbox])
        end
    end
    log("hmm")
end
